...
**Recommendation for a sell use case:** do **not** load the entire `Portfolio` with every `Holding` and every `Lot`. Either narrow the load (option 1) or split the aggregate (option 2). Loading the full portfolio for a single-ticker sell is the textbook example of an oversized aggregate: it inflates transaction time, increases lock contention across unrelated tickers, and produces tail-latency that grows with portfolio size — none of which is justified by an invariant.
