...
També hi ha detalls formals que convindria corregir abans de la defensa: `desacoplada` hauria de ser `desacoblada`; `Generarlitat` hauria de ser `Generalitat`; `tecnología` hauria de ser `tecnologia`; `necesita` hauria de ser `necessita`; `d´ús` hauria de ser `d'ús`; la numeració de diapositives 3 i 4 apareix duplicada; i les notes del presentador de les diapositives 1 i 2 contenen informació sobre PICA que correspon millor a les diapositives 5 o 6.
...
La diapositiva funciona. Corregiria `desacoplada` per `desacoblada`. Si es vol reforçar el vincle amb el pla docent, es pot afegir en text petit: `Pla docent 103322: clean/hexagonal architecture, ports i adaptadors, mapping, mòduls i DDD`. No cal citar tot el pla docent; amb una referència discreta n'hi ha prou.
